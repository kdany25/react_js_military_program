...
import { deleteRoadPlan, getRoadPlan } from "../../redux/apiCalls";
...
export default function RoadPlanList() {
	...
	const handleSort = (ev) => {
		...
		for (let p = 5; p > 0; p--) {
			// Priority loop
			for (let i = 0; i < roadPlans.length; i++) {
				const plan = roadPlans[i];
		...
		<>
			<div className="userList">
				...
				<div className="sort-wrapper">
					<p>
						Calculating labor and material cost, creating timetables
						with targets goals
					</p>
				...
				<div className="header">
					...
					<h5>
						...
						<strong>{totalCost.toLocaleString()} Rwf</strong>{" "}
					</h5>
				</div>
				<DataGrid
					rows={roadPlans}
				...
				<div className="modal-wrapper">
					...
					<section>
						<header className="header">
							<p>
								<strong>{modalData.length}</strong> of{" "}
								<strong>{roadPlans.length}</strong> activities
								are eligible, worth{" "}
								<strong>{showModal.total} M</strong> of a{" "}
								<strong>{budget} M</strong> Budget
							</p>
							<CloseOutlined
								className="closeBtn"
								onClick={toggleModal}
